...
2. Launch Blind Hunter and Edge Case Hunter in parallel without prior conversation context. If `{review_mode}` = `"full"`, include the Acceptance Auditor in the same parallel launch. If subagents are not available, generate prompt files in `{implementation_artifacts}` for each applicable reviewer role and HALT. Ask the user to run each in a separate session (ideally a different LLM) and paste back the findings. When findings are pasted, resume from this point and proceed to step 3.
   ...
   - **Acceptance Auditor** (only if `{review_mode}` = `"full"`) — prompt:
     > You are an Acceptance Auditor. Review the provided diff against `{spec_file}` and any loaded context docs. Check for: violations of acceptance criteria, deviations from spec intent, missing implementation of specified behavior, contradictions between spec constraints and actual code. Also check: for every error-handling behavior the spec's Dev Notes document as an intent or promise (e.g. "X failure returns Y status", "rollback on error"), verify the diff actually implements it with evidence (a test, or a targeted read of the implementing code) — a documented intent is not proof of implementation; flag any promise that is declared but not coded. Output findings as a Markdown list. Each finding: one-line title, which AC/constraint it violates, and evidence from the diff.
     >
     > Diff:
     > {diff_output}

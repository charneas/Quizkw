...
class TestAdvanceToPhase3:
    def test_rejects_from_manche_1(self, test_client, sample_game_session, host_headers):
        """La Manche 3 ne peut pas démarrer directement depuis la Manche 1."""
        response = test_client.post(f"/games/{sample_game_session.code}/advance-to-phase3", headers=host_headers)

        assert response.status_code == 400
        assert "Manche 2" in response.json()["detail"]

    def test_advances_from_manche_2(self, test_client, db_session, sample_game_session, host_headers):
    ...
    def test_idempotent_when_already_in_manche_3(self, test_client, db_session, sample_game_session, host_headers):
    ...
    def test_idempotent_even_when_game_no_longer_active(self, test_client, db_session, sample_game_session, host_headers):
    ...
    def test_rejects_from_manche_2_when_game_not_active(self, test_client, db_session, sample_game_session, host_headers):
